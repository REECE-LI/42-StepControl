...
void Stockpile_Flash_Data_Read_Data(Stockpile_FLASH_Typedef *stockpile, void *data, uint32_t num) {

    HAL_FLASH_Unlock();
    uint16_t *dataTemp = data;
    for (uint32_t i = 0; i < num; i++) {
        dataTemp[i] = FLASH_ReadHalfWord(stockpile->begin_add + i * 2);
    }
    HAL_FLASH_Lock();
}


void Stockpile_Flash_Data_QWrite_Data16(Stockpile_FLASH_Typedef *stockpile, uint16_t *data, uint32_t num) {
    Stockpile_Flash_Data_Empty(stockpile);
    Stockpile_Flash_Data_Begin(stockpile);
    Stockpile_Flash_Data_Write_Data16(stockpile, data, num);
    Stockpile_Flash_Data_End(stockpile);
}
/*************************************************************** Flash_End ***************************************************************/
/*************************************************************** Flash_End ***************************************************************/
/*************************************************************** Flash_End ***************************************************************/
